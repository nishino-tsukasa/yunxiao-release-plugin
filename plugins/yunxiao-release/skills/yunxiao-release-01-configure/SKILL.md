...
2. 读取项目共享配置、用户级 `global-defaults.json`、`global-repositories.json` 和当前 Git remote，按“项目配置 > 全局仓库配置 > 全局默认配置 > 插件默认值”合并。全局仓库项支持项目配置的全部字段；项目文件可以缺失或只含覆盖字段。全局默认配置禁止包含 `repositoryId`。
...
12. 全局迁移后重新读取当前 remote 的有效配置，逐字段确认原项目共享配置均已被覆盖。`projectType=frontend` 时保留项目内 `.agents/yunxiao-release.json`；`projectType=backend` 时仅在完整性检查通过后删除该项目共享配置。不得删除成员配置、运行状态或评论文件；无法确认项目类型时保留项目配置并列入待处理问题。
