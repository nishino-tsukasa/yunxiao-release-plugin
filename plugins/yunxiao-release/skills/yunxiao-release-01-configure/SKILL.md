...
2. 读取项目共享配置、用户级 `global-defaults.json`、`global-repositories.json` 和当前 Git remote，按“项目配置 > 全局仓库配置 > 全局默认配置”合并。全局仓库项支持项目配置的全部字段；项目文件可以缺失或只含覆盖字段。全局默认配置只允许跨仓库一致的组织级、存储和执行字段，禁止写入仓库 ID、remote、分支、评审人、发版文件、验证命令或环境步骤；插件不补组织或项目策略默认值。
...
5. 缺少 `organizationId` 时调用 `get_current_organization_info` 和 `get_user_organizations`；缺少 `repositoryId` 时从标准化 remote 提取仓库路径，用 `list_repositories` 准备候选并用 `get_repository` 精确核实。不得从 remote 猜 ID。
   MR 目标分支、环境目标分支、提交规则、流水线及 Client 触发条件均从输入来源核实后显式写入仓库配置的 `environments`；插件不根据项目名称或类型推断这些值。读取旧配置时可保留旧字段，由兼容层转换，不主动改写项目文件。
