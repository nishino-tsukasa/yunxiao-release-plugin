...
3. 对每个仓库读取显式配置的 `remoteName`、`fatTargetBranch`、`commitMessagePattern` 和 `clientDetection`；任何字段缺失都停止，不从仓库名称、目录名称或项目类型推断。
...
认证从 `~/.config/yunxiao-release/credentials.env` 读取；组织 ID、流水线目录、发现规则和执行参数从全局默认配置的 `fatFlow` 读取，仓库特定行为从全局仓库配置读取。插件目录不保存组织或项目专属流水线信息。
